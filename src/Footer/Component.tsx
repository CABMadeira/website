  ...
  return (
    <footer className="bg-[hsl(var(--primary))] py-10">
      <div className="text-center">
        ...
        {/* Footer Navigation Links (from CMS) */}
        {/* <div className="my-4 flex flex-wrap justify-center gap-6 text-sm text-[hsl(var(--secondary))]">
          {navItems.map(({ link }, index) => (
            <CMSLink
              key={index}
              className="hover:text-[hsl(var(--yellow))] block duration-150"
              {...link}
            />
          ))}
        </div> */}
        ...
        <ul className="flex justify-center mt-5 space-x-5 text-[hsl(var(--secondary))]">
          ...
          {/* Add remaining social icons here */}
        </ul>
      </div>
    </footer>
  )
